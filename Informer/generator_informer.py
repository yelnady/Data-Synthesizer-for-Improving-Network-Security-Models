...
model.load_state_dict(torch.load('class_all_weights_informer'))
model.eval()
print()
    ...
    for n in range(n_samples):
        ...
        for t in range(max_length-n_seed): # Loop until 400 timesteps
            ...
            if S == datapoint_len :
                ...
                mask =  torch.cat((mask,torch.full((1,max_length-S),True)))
                break
        ...
        if (n%1000==0):
             np.savez('generated_informer',X=generated_dataset_X,masks= resulted_masks,Y=generated_dataset_Y)
