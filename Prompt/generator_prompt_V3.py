...
class TimeSeriesTransformer(nn.Module):
    ...
    def forward(self, src, prompt, src_mask,padding_mask):
        p = self.PromptLinear(prompt) * math.sqrt(self.d_model) # Prompt Input: [Batch Size, n_classes ] 
...
ck = torch.load('lightning_logs/version_33/checkpoints/epoch=230-step=44351.ckpt')['state_dict']
...
def generate_dataset(X,Y,Y_labels,masks,n_seed,n_samples,max_length,n_metadata=1):
    ...
    for n in range(n_samples):
        ...
        datapoint,y,y_label,mask = get_n_samples(X,Y,Y_labels,masks,n_samples=1) # The first 10 timesteps of just one sample
...
#         print(datapoint.shape)
        ...
        generated_dataset_X.append(datapoint.permute(1,0,2)[:,n_metadata:].squeeze().detach().numpy())
        generated_dataset_Y.append(y_label.item())
        if (n%100==0):
            print('{}/{}'.format(n,n_samples))
#         if (n%1000==0):
        np.savez('npz_transformer_prompt_V3_seed2',X=generated_dataset_X,masks= resulted_masks,Y=generated_dataset_Y)
