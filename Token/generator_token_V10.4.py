...
model.load_state_dict(torch.load('W_transformer_token_V10.4'))
model.eval()
print()
...
def generate_dataset(X,Y,masks,n_seed,n_samples,max_length):
    for n in range(n_samples):
        ...
        for t in range(max_length-n_seed): # Loop until 400 timesteps
            ...
            if F.softmax(nlp_predicted.squeeze()).argmax(1)[-1].item() == n_classes_disc-2:
        ...
        if ((n+1)%1000==0):
             np.savez('npz_transformer_token_V10.4',X=generated_dataset_X,masks= resulted_masks,Y=generated_dataset_Y)
