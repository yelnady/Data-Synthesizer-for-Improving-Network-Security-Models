...
def round_torch(arr, decimal):
    x = torch.round(arr * 10**decimal) / (10**decimal)
    return x 
...
dataset = torch.utils.data.TensorDataset(real_train_X_nlp, targets_nlp.long(), real_train_masks)
...
class TimeSeriesTransformer(pl.LightningModule):
    ...
    def training_epoch_end(self, outputs):
        if((self.current_epoch+1)%100==0):
            torch.save(self.state_dict(), 'W_transformer_token_V11')
