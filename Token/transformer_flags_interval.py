...
# Decreasing Range is [1,0.7], END TOKEN Range is [0.7, 0.5]
...
torch.linspace(Decrease_max,Decrease_min, steps = (5-1))
...
for i,after_last in enumerate(real_train_lengths):
    magic_row = np.zeros((2500))
    magic_row[:after_last] = np.linspace(Decrease_max,Decrease_min,  (after_last)) 
    magic_row[after_last]  = END_TOKEN
...
magic_rows_torch = torch.Tensor(magic_rows).unsqueeze(2)


# In[20]:


magic_rows_torch[200,:20], real_train_lengths[200]


# In[23]:


real_train_X = torch.cat((real_train_X, magic_rows_torch),2)
...
class TimeSeriesTransformer(pl.LightningModule):
    ...
    def training_epoch_end(self, outputs):
        if((self.current_epoch+1)%100==0):
            torch.save(self.state_dict(), 'W_transformer_allV3_new')
            torch.save(self, 'transformer_allV3_new')
...
# model.load_state_dict(torch.load("W_transformer_allV3"))
...
# Epoch Loss: 0.005757194012403488
# Epoch Loss: 0.0018120573367923498
# Epoch Loss: 0.0014234031550586224
# Epoch Loss: 0.001140992040745914
# Epoch Loss: 0.0009430662030354142
# Epoch Loss: 0.0008919127285480499


# In[ ]:


# Transformer_all_V3_new without corrected positional encoding we got mse = 0.00020625354954972863 after 400 epochs
