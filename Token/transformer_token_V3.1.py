...
# V3.1 : Similar to V3 --> But we inverse transformed all data 
...
torch.min(real_train_X_inverse),torch.max(real_train_X_inverse)
...
real_val_X_inverse = logit(real_val_X)
torch.min(real_val_X_inverse),torch.max(real_val_X_inverse)
...
for i,after_last in enumerate(real_train_lengths):
    real_train_X_inverse[i,after_last]  = end_token.clone()
...
for i,after_last in enumerate(real_val_lengths):
    real_val_X_inverse[i,after_last]  = end_token.clone()
...
real_train_X_inverse = real_train_X_inverse[:,:400]
...
dataset = torch.utils.data.TensorDataset(real_train_X_inverse, targets, real_train_masks)
...
real_val_X_inverse = real_val_X_inverse[:,:400]

S = real_val_X.size(1)

params_dataloader = {'shuffle': False,'num_workers':8 ,'batch_size':128} # No need to shuffle rn, they are all the same class
dataset = torch.utils.data.TensorDataset(real_val_X_inverse, targets, real_val_masks)
...
class TimeSeriesTransformer(pl.LightningModule):
    ...
    def training_epoch_end(self, outputs):
        if((self.current_epoch+1)%100==0):
            torch.save(self.state_dict(), 'W_transformer_token_V3.1')
