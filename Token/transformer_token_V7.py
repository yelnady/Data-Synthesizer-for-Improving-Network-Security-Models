...
dataset = torch.utils.data.TensorDataset(real_val_X, targets, real_val_auxiliary.long(), real_val_masks)
...
class TimeSeriesTransformer(pl.LightningModule):
    ...
    def training_step(self, batch, batch_idx):
        ...
        diff2 = nn.CrossEntropyLoss(reduction='none')(aux_predicted.reshape(-1, 3),aux_real.flatten())
        flags = (~padding_mask).float().flatten()
        aux_loss  = diff2*flags
        aux_loss = torch.sum(diff2) / torch.sum(flags)
    ...
    def training_epoch_end(self, outputs):
        if((self.current_epoch+1)%100==0):
            torch.save(self.state_dict(), 'W_transformer_aux_V7')
