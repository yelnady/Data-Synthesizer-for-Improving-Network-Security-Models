...
model.load_state_dict(torch.load('W_transformer_token_V6'))
model.eval()
print()
...
def generate_dataset(X,Y,masks,n_seed,n_samples,max_length):
    for n in range(n_samples):
        ...
        for t in range(max_length-n_seed): # Loop until 400 timesteps
            ...
            #print(F.softmax(aux_predicted[-1]).argmax().item(),aux_predicted[-1])
            if F.softmax(aux_predicted[-1]).argmax().item() == 1:
        ...
        if ((n+1)%1000==0):
             np.savez('npz_transformer_token_V6',X=generated_dataset_X,masks= resulted_masks,Y=generated_dataset_Y)
