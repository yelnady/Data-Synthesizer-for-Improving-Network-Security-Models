...
class TimeSeriesTransformer(nn.Module):
    ...
    def __init__(self, n_features=10, d_model=512, n_heads=8, n_hidden=512, n_layers=8, dropout=0.0):
        super().__init__()
...
model.load_state_dict(torch.load('class_transformer_flags1'))
model.eval()
print()
    ...
    for n in range(n_samples):
        ...
        for t in range(max_length-n_seed): # Loop until 400 timesteps
            ...
            if  one_new_timestep[...,-1] <=0.5 : 
                ...
                mask =  torch.cat((mask.cpu(),torch.full((1,max_length-S),True)),1)
                break
        ...
        if (n%1000==0):
             np.savez('class_all_generated_flags',X=generated_dataset_X,masks= resulted_masks,Y=generated_dataset_Y)
