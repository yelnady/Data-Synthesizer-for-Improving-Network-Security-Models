...
# V3 was as V2 but changing how calculating the loss
# V5 is Same as V3 but we are weighting the loss
...
# model.load_state_dict(torch.load('W_transformer_flags1_V6_1024'))
ck = torch.load('lightning_logs/version_64/checkpoints/epoch=499-step=191499.ckpt')['state_dict']
...
#                 print(S,datapoint_len)
        ...
        if ((n+1)%1000==0):
             np.savez('npz_transformer_flags1_V8',X=generated_dataset_X,masks= resulted_masks,Y=generated_dataset_Y)
...
generate_dataset(real_train_X0 ,real_train_Y_labels0 ,padding_mask0,n_seed=n_seed,n_samples=real_train_X0.size(0)*2,max_length=max_length)
