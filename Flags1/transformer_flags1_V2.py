...
## V2 differs from V1 in the d_model is 200 rather than 512, so I can generate fast without waiting for hours.
...
def train(model,train_dataloader,src_mask,n_epochs):
    ...
    for epoch in range(n_epochs):
        ...
        for idx,(X,target,padding_mask) in enumerate(train_dataloader):
            ...
            Y_predicted = model(X.to(device),src_mask.to(device),padding_mask.to(device)).cpu()
        ...
        if (epoch%100==0):
            torch.save(model.state_dict(), 'W_transformer_flags1_V2')
...
train(model,train_dataloader,src_mask,n_epochs=200)
